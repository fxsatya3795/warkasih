...
function syncCashierOrdersFromStorage() {
    try {
        const storedOrders = JSON.parse(localStorage.getItem("adminOrders") || "[]");
        cashierOrders = storedOrders;
        renderCashierOrders();
    } catch (error) {
        console.error(error);
    }
...
function renderNotifications(incoming) {
    const visibleOrders = incoming || [];
    const badge = document.getElementById("notificationBadge");
    const list = document.getElementById("notificationList");

    if (badge) badge.textContent = String(visibleOrders.length);

    if (!list) return;

    if (!visibleOrders.length) {
        list.innerHTML = "<p style='padding: 16px; margin: 0; color: #718896;'>Tidak ada pesanan baru.</p>";
        return;
    }

    list.innerHTML = visibleOrders.map((order) => {
        const statusText = isPaymentReceived(order)
            ? "✅ Siapkan pesanan"
            : "⏳ Menunggu pembayaran";
        return `
            <button
                ...
                onclick="openPOS('${escapeCashierHtml(order.id)}'); toggleCashierNotification();"
            >
                <strong>${escapeCashierHtml(order.number || order.id)}</strong>
                <span>${statusText} • ${escapeCashierHtml(order.customer || "Pelanggan")}</span>
            </button>
        `;
    }).join("");
}
...
function renderReceipt(order) {
    ...
    const items = orderItems(order).map((item) => `
        <div class="receipt-item-row">
            <span>${escapeCashierHtml(item.name || item.productName || "Produk")} x ${itemQuantity(item)}</span>
            <strong>${cashierCurrency(itemPrice(item) * itemQuantity(item))}</strong>
        </div>
    `).join("");

    const subtotal = orderTotal(order);
    const discount = Number(order.discount || 0);
    const total = Number(order.paidTotal || order.total || subtotal - discount || 0);
    const paidAmount = Number(order.paidAmount || order.paidTotal || total || 0);
    const change = Number(order.change || Math.max(0, paidAmount - total));

    content.innerHTML = `
        <div id="printArea" class="receipt-print-area">
            <div class="receipt-header">
                <strong>Struk Pembelian</strong>
                <span>Kasir Jemaat</span>
            </div>

            <div class="receipt-meta">
                <div><span>No. Pesanan</span><strong>${escapeCashierHtml(order.number || order.id)}</strong></div>
                <div><span>Pelanggan</span><strong>${escapeCashierHtml(order.customer || "-")}</strong></div>
                <div><span>Waktu</span><strong>${new Date(order.paidAt || order.createdAt || Date.now()).toLocaleString("id-ID")}</strong></div>
            </div>

            <div class="receipt-divider"></div>

            <div class="receipt-items">
                ${items || "<p>Belum ada item.</p>"}
            </div>

            <div class="receipt-divider"></div>

            <div class="receipt-summary">
                <div class="receipt-row"><span>Subtotal</span><strong>${cashierCurrency(subtotal)}</strong></div>
                <div class="receipt-row"><span>Diskon</span><strong>${cashierCurrency(discount)}</strong></div>
                <div class="receipt-row total"><span>Total</span><strong>${cashierCurrency(total)}</strong></div>
                <div class="receipt-row"><span>Pembayaran</span><strong>${cashierCurrency(paidAmount)}</strong></div>
                <div class="receipt-row"><span>Kembalian</span><strong>${cashierCurrency(change)}</strong></div>
            </div>
...
function printCashierReceipt() {
    ...
    const originalContent = document.getElementById("receiptContent")?.innerHTML;
    if (originalContent) {
...
function updateCashierProfileAvatar(imageSource) {
    const avatar = document.getElementById("cashierProfileAvatar");
...
function bindCashierProfileUpload(userEmail = null) {
    ...
    const input = document.getElementById("cashierProfileInput");
    if (!input) return;
    input.addEventListener("change", (event) => {
        ...
        reader.onload = () => {
            ...
            updateCashierProfileAvatar(imageData);
    ...
    if (savedImage) updateCashierProfileAvatar(savedImage);
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    bindCashierProfileUpload(userData.email);
    loadCashierData();
    updateCashierClock();
    setInterval(updateCashierClock, 1000);
    window.addEventListener("storage", (event) => {
        if (event.key === "adminOrders") syncCashierOrdersFromStorage();
    });
    window.addEventListener("adminOrdersUpdated", syncCashierOrdersFromStorage);
});
